...
class RNN_Model(object):
    def __init__(self, config, is_training=True):
        ...
        # 返回的是两个维度是batch_size * hidden_neural_size的零向量元组, 其实就是LSTM初始化的c0, h0向量
        ...
        loss_summary = scalar_summary("loss", self.cost)
        accuracy_summary = scalar_summary("accuracy_summary", self.accuracy)
        ...
        for g, v in zip(grads, tvars):
            if g is not None:
                grad_hist_summary = histogram_summary("{}/grad/hist".format(v.name), g)
                sparsity_summary = scalar_summary("{}/grad/sparsity".format(v.name), tf.nn.zero_fraction(g))
                grad_summaries.append(grad_hist_summary)
                grad_summaries.append(sparsity_summary)
        self.grad_summaries_merged = merge_all_summaries(grad_summaries)

        self.summary = merge_all_summaries([loss_summary, accuracy_summary, self.grad_summaries_merged])
